...
class ObjModel:
    ...
    def load_obj(path):
        ...
        logger.info(f" Cargando modelo: {os.path.basename(path)}")
        ...
        logger.info(f" Modelo cargado: {len(vertices)} vértices, {len(faces)} caras")
        
        if uvs is not None:
            logger.info(f"   Texturas: {len(uvs)} coordenadas UV")
        else:
            logger.info(f"   Texturas: NO DISPONIBLES")
            
        if normals is not None:
            logger.info(f"   Normales: {len(normals)} normales de vértice")
        else:
            logger.info(f"   Normales: NO DISPONIBLES - se generarán automáticamente")
    ...
    def generate_normals(self):
        ...
        logger.info(" Generando normales planas...")
        ...
        logger.info(f" Normales generadas: {len(self.normals)} normales")
    ...
    def print_info(self):
        ...
        logger.info(f"   Vértices: {len(self.vertices)}")
        logger.info(f"   Caras: {len(self.faces)}")
        logger.info(f"   Coordenadas UV: {'SÍ' if self.has_texture_coordinates() else 'NO'}")
        logger.info(f"   Normales: {'SÍ' if self.has_normals() else 'NO'}")
        ...
        logger.info(f"   Triángulos: {tri_count}")
        logger.info(f"   Cuadriláteros: {quad_count}")
        logger.info(f"   Otros: {other_count}")
        
        if self.uvs is not None:
            logger.info(f"   UVs únicas: {len(self.uvs)}")
        
        if self.normals is not None:
            logger.info(f"   Normales únicas: {len(self.normals)}")
...
if __name__ == '__main__':
    ...
    if model.has_normals():
        ...
        for i in range(min(5, len(model.normals))):
            logger.info(f"   Normal {i}: [{model.normals[i][0]:.3f}, {model.normals[i][1]:.3f}, {model.normals[i][2]:.3f}]")
